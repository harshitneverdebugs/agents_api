...
@app.route('/api/today-report', methods=['GET'])
def get_today_report():
    """API endpoint to get today's report with user-provided or default readings"""

    # Check if 'default_readings' is provided in the request
    default_readings = request.args.get('default_readings')

    # If no readings are provided, prompt the user (useful for local testing)
    if not default_readings:
        default_readings = input("Enter default Bazi readings for today: ") or "Default Bazi readings for today"
    ...
    # Create agents and run the tasks
    crew = create_agents(default_readings, current_date)
    result = crew.kickoff(inputs={"readings": default_readings, "current_date": current_date})
